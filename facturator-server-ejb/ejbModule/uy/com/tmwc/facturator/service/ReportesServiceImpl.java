...
public class ReportesServiceImpl implements ReportesService {
	...
	public TableReportResult getReporteStockPrecios(ReportParameters parameters) {
		...
		if (!(cotizaciones instanceof Cotizaciones)) {
			throw new RuntimeException("El parametro 'cotizaciones' (" + cotizaciones + ") no es válido");
		}
		Object moneda = table.get("moneda");
		if (!(moneda instanceof String)) {
			throw new RuntimeException("El parametro 'moneda' (" + moneda + ") no es válido");
		}
		Object filtro = table.get("filtro");
		if (filtro != null) {
			if (!(filtro instanceof String)) {
				throw new RuntimeException("El parametro 'filtro' (" + filtro + ") no es válido");
			}
		}
		Object desde = table.get("desde");
		if (desde != null) {
			if (!(desde instanceof String)) {
				throw new RuntimeException("El parametro 'desde' (" + desde + ") no es válido");
			}
		}
		Object hasta = table.get("hasta");
		if (hasta != null) {
			if (!(hasta instanceof String)) {
				throw new RuntimeException("El parametro 'hasta' (" + hasta + ") no es válido");
			}
		}
		...
		if (proveedores != null) {
			if (!(proveedores instanceof String)) {
				throw new RuntimeException("El parametro 'proveedores' (" + proveedores + ") no es válido");
			}
		}
		...
		if (familias != null) {
			if (!(familias instanceof String)) {
				throw new RuntimeException("El parametro 'familias' (" + familias + ") no es válido");
			}
		}
		...
		if (filtrarCeros != null) {
			if (!(filtrarCeros instanceof Boolean)) {
				throw new RuntimeException("El parametro 'filtrarCeros' (" + filtrarCeros + ") no es válido");
			}
		}

		Object filtrarNegativos = table.get("filtrarNegativos");
		if (!(filtrarNegativos instanceof Boolean)) {
			throw new RuntimeException("El parametro 'filtrarNegativos' (" + filtrarNegativos + ") no es válido");
		}

		Object mostrarCosto = table.get("mostrarCosto");
		if (mostrarCosto != null) {
			if (!(mostrarCosto instanceof Boolean)) {
				throw new RuntimeException("El parametro 'mostrarCostos' (" + mostrarCosto + ") no es válido");
			}
		}
		...
		if (mostrarPrecioFab != null) {
			if (!(mostrarPrecioFab instanceof Boolean)) {
				throw new RuntimeException("El parametro 'mostrarPrecioFabrica' (" + mostrarPrecioFab
						+ ") no es válido");
			}
		}
		...
		List<Object[]> dbData = reportesLibra.stockPrecio(listas, (String) filtro, (String) proveedores,
				(String) familias, (String) desde, (String) hasta, Boolean.TRUE.equals(filtrarCeros),
				Boolean.TRUE.equals(filtrarNegativos));
		...
		Boolean mostrarPrecios = esSupervisor || Usuario.USUARIO_ADMINISTRADOR.equals(permisoId)
		...
		for (Object[] dbDatum : dbData) {
			Object[] processed = new Object[3 + listas.length
					+ (mostrarPrecios ? ((Boolean) mostrarCosto ? 1 : 0) + ((Boolean) mostrarPrecioFab ? 1 : 0) : 0)];
			int col = 0;
			...
			if (mostrarPrecios && Boolean.TRUE.equals(mostrarCosto)) {
				...
				processed[col++] = new LogicaCotizacion(Moneda.CODIGO_MONEDA_PESOS, Moneda.CODIGO_MONEDA_DOLAR)
						.getMontoMayorCotizacion(fprecio, mprecio, precio, (String) moneda,
								(Cotizaciones) cotizaciones, Boolean.FALSE, getTipoCambioFunction);
			}
	...
	public byte[] generarLiquidacionAfilados(Date desde, Date hasta, BigDecimal value) throws IOException {
		Map<String, ArrayList<ParticipacionAfilador>> participaciones = documentoDAOService.getParticipacionesAfilados(
				desde, hasta, value);
	...
	public byte[] generarListadoControlLineasVenta(Date fechaDesde, Date fechaHasta, BigDecimal rentaMinima,
		...
		List<Documento> docs2 = documentoDAOService.getDocumentos(fechaDesde, fechaHasta,
		...
		List<Documento> docs4 = documentoDAOService.getDocumentos(fechaDesde, fechaHasta,
				Moneda.CODIGO_MONEDA_DOLAR_ASTER);
	...
	private CsvWriter generarListadoControlLineasVentaW(List<Documento> docs, CsvWriter w, BigDecimal rentaMinima,
			BigDecimal rentaMaxima) throws IOException {
		NumberFormat formatter = NumberFormat.getNumberInstance(new Locale("ES"));
		...
		for (Documento doc : docs) {
			for (LineaDocumento linea : doc.getLineas().getLineas()) {
				...
				if (renta.compareTo(rentaMinima) < 0 || renta.compareTo(rentaMaxima) > 0) {
					...
					w.write(linea.getArticulo() != null ? linea.getArticulo().getCodigo() + " - "
					...
					if ((linea.getArticulo() == null) && !("".equals(linea.getConceptoIdLin()))) { // No deberían haber documentos de gasto en los comprobantes de venta
						...
						BigDecimal utilidad = linea.getPorcentajeUtilidad() != null ? linea.getPorcentajeUtilidad()
								.setScale(1, RoundingMode.HALF_EVEN) : BigDecimal.ZERO;
	...
	public byte[] generarLiquidacionVendedores(Date desde, Date hasta, String[] compsIncluidos, String[] compsExcluidos)
			throws IOException {
		...
		List<ParticipacionVendedor> participaciones = documentoDAOService.getParticipaciones(desde, hasta,
				compsIncluidos, compsExcluidos);

		List<TipoCambio> tipoCambios = reportesLibra.getUltimosTipoCambios();
		GetTipoCambio getTipoCambioFunction = new LogicaCotizacion.InMemoryGetTipoCambio(tipoCambios);
		LogicaCotizacion lc = new LogicaCotizacion(Moneda.CODIGO_MONEDA_PESOS, Moneda.CODIGO_MONEDA_DOLAR,
		...
		for (ParticipacionVendedor participacionVendedor : participaciones) {
			...
			try {
				...
				System.err.println("Vendedor tiene un codigo no numérico: "
						+ participacionVendedor.getVendedor().getCodigo());
				e.printStackTrace();
				continue;
			}
			...
			String monedaDoc = Moneda.getCodigoMonedaNoAster(participacionVendedor.getDocumento().getMoneda()
					.getCodigo());
			...
			if (vendedoresDist.contains(participacionVendedor.getVendedor().getCodigo())) {
				...
				BigDecimal part = participacionVendedor.getCuotaparte(rentaDistribuidor) != null ? participacionVendedor
						.getCuotaparte(rentaDistribuidor) : BigDecimal.ZERO;

				if (rentaDistribuidor != null) {
					if (creditoAlVendedor.containsKey(codigoVen)) {
						rentaDistribuidor = creditoAlVendedor.get(codigoVen).add(
								rentaDistribuidor.divide(coti, 2, RoundingMode.HALF_UP));
					} else {
				...
				BigDecimal cuotaparte = participacionVendedor.getCuotaparte(renta) != null ? participacionVendedor
						.getCuotaparte(renta) : BigDecimal.ZERO;
				if (creditoAlVendedor.containsKey(codigoVen)) {
		...
		try {
			for (ParticipacionVendedor participacionVendedor : participaciones) {
				...
				try {
					Integer.parseInt(codigoVendedorStr);
				} catch (NumberFormatException e) {
					System.err.println("Vendedor tiene un código no numerico: " + codigoVendedorStr);
					e.printStackTrace();
					continue;
				}
				LiquidacionVendedor lv = ensureLiquidacion(codigoVendedorStr, liqByCodigo, lc, vendedoresDist,
						vendedoresJunior, vendedoresSenior);
			...
			for (LiquidacionVendedor liquidacionVendedor : liqs) {
				...
				liquidacionVendedor.totalizar(importeNetoVendido.get(codigo), rentaBruta.get(codigo),
						creditoAlVendedor.get(codigo), participacion.get(codigo));
			}
	...
	private LiquidacionVendedor ensureLiquidacion(String codigoVendedorStr,
			HashMap<Integer, LiquidacionVendedor> liqByCodigo, LogicaCotizacion cotizacion, ArrayList<String> dist,
			ArrayList<String> junior, ArrayList<String> senior) throws IOException {

		int codigoVendedor = 0;
		try {
			codigoVendedor = Integer.parseInt(codigoVendedorStr);
		} catch (NumberFormatException e) {
			System.err.println("Vendedor tiene un código no numérico: " + codigoVendedorStr);
			e.printStackTrace();
		}
		...
		File file = new File(
				System.getProperty("jboss.home.url").substring(6)
						+ "/server/default/deploy/facturator-server-ear.ear/facturator-server.war/liquidacion/output/liquidacion-"
						+ codigoVendedor + ".csv"); // txt para que abra el asistente de importacion de excel
		...
		/*
		 * else { System.err.println("Codigo de vendedor no puede asignarse a un tipo: " + codigoVendedor); return null; }
		 */
		lv.inicializar();
	...
	private static abstract class LiquidacionVendedor {
		...
		protected void totalizar(BigDecimal param1, BigDecimal param2, BigDecimal param3, BigDecimal param4)
				throws IOException {
			writer.write("");
			writer.write("");
			writer.write("");
		...
		public void addParticipacion(ParticipacionVendedor pv) throws IOException {
			...
			if (!documento.getComprobante().getCodigo().equals("98")) {
				writer.write(formatter.format(documento.getVentaNeta() != null ? documento.getVentaNeta()
						: BigDecimal.ZERO));
			} else {
				writer.write(formatter.format(documento.getTotal() != null ? documento.getTotal().negate()
						: BigDecimal.ZERO));
			}
		...
		if (!documento.getComprobante().getCodigo().equals("98")) {
			...
			if (entrega != null) {
				...
				if (costoMonedaDoc == null) {
					System.err.println("No se pudo realizar cambio: " + entrega.getMonedaCosto() + "->"
							+ monedaNormalizadaDoc + ", monto: " + entrega.getCosto());
					return null;
				}
	...
	private static class LiquidacionJunior extends LiquidacionVendedor {
		...
		protected void subAddParticipacion(ParticipacionVendedor pv) throws IOException {
			...
			BigDecimal porcentajeComision = pv.getVendedor().getPorcentajeComision(
					pv.getDocumento().getPreciosVenta().getCodigo());
			writer.write(formatter.format(porcentajeComision != null ? porcentajeComision : BigDecimal.ZERO));
			writer.write(porcentajeComision == null ? null : formatter.format(porcentajeComision.multiply(
					pv.getCuotaparte(pv.getDocumento().getVentaNeta())).divide(Maths.ONE_HUNDRED)));
		}
		...
		protected void totalizando(BigDecimal param1, BigDecimal param2, BigDecimal param3) throws IOException {
			...
			writer.write("");
			if (param2 != null)
				writer.write(formatter.format(param2));
			if (param3 != null)
				writer.write(formatter.format(param3));
		}
	...
	private static class LiquidacionDistribuidor extends LiquidacionVendedor {
		...
		protected void subAddParticipacion(ParticipacionVendedor pv) throws IOException {
			...
			writer.write(formatter.format(rentaDistribuidor != null && pv.getCuotaparte(rentaDistribuidor) != null ? pv
					.getCuotaparte(rentaDistribuidor) : BigDecimal.ZERO));
		}
	...
	private static class LiquidacionSenior extends LiquidacionVendedor {
		...
		protected void subAddParticipacion(ParticipacionVendedor pv) throws IOException {
			...
			writer.write(formatter.format(renta != null && pv.getCuotaparte(renta) != null ? pv.getCuotaparte(renta)
					: BigDecimal.ZERO));
		}
