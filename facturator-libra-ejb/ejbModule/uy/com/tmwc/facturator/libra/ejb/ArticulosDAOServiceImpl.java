...
public class ArticulosDAOServiceImpl extends ServiceBase implements ArticulosDAOService {
	...
	private static final String ARTICULOS_SUBQUERY = "FROM Articulo a WHERE a.id.empId = :empId "
			+ "AND (a.activo = :activo OR :activo is null) ";

	private static final String ARTICULO_PRECIO = "FROM ArticuloPrecio a WHERE a.id.empId = :empId "
			+ "AND (a.id.artId = :artId) ";

	public String persist(Articulo e) {
		DozerBeanMapper mapper = this.mapService.getDozerBeanMapper();
		uy.com.tmwc.facturator.libra.entity.Articulo articulolibra = mapper.map(e,
	...
	public void merge(Articulo e) {
		...
		uy.com.tmwc.facturator.libra.entity.Articulo libraArticulo = mapper.map(e,
				uy.com.tmwc.facturator.libra.entity.Articulo.class);
	...
	public Boolean delete(Articulo e) {
		DozerBeanMapper mapper = this.mapService.getDozerBeanMapper();
		uy.com.tmwc.facturator.libra.entity.Articulo libraArticulo = (uy.com.tmwc.facturator.libra.entity.Articulo) mapper
				.map(e, uy.com.tmwc.facturator.libra.entity.Articulo.class);
	...
	public List<ArticuloDTO> queryArticulos(ArticuloQuery query) {
		final StringBuilder sb = new StringBuilder("SELECT new uy.com.tmwc.facturator.dto.ArticuloDTO(a.codigo, "
				+ "a.nombre, a.familiaId, a.codigoOrigen) ");
	...
	public List<ArticuloPrecio> queryArticuloPrecio(String articulo) {
		final StringBuilder sb = new StringBuilder("SELECT new uy.com.tmwc.facturator.entity.ArticuloPrecio("
				+ "a.id.artId, a.id.precioBaseId, a.precio, a.mndIdPrecio, a.precioBaseConIVA"
				+ ") ");
	...
	public void updateArticuloPrecio(ArticuloPrecio e) {
		...
		uy.com.tmwc.facturator.libra.entity.ArticuloPrecio artPrecioEntity = em.find(
				uy.com.tmwc.facturator.libra.entity.ArticuloPrecio.class, articuloPrecioPK);
	...
	private void persist(ArticuloPrecio e) {
		...
		uy.com.tmwc.facturator.libra.entity.Articulo articuloEntity = em.find(
				uy.com.tmwc.facturator.libra.entity.Articulo.class, articuloPK);
		...
		uy.com.tmwc.facturator.libra.entity.ArticuloPrecio articuloPrecioLibra = mapper.map(e,
				uy.com.tmwc.facturator.libra.entity.ArticuloPrecio.class);
		ArticuloPrecioPK articuloPrecioPK = new ArticuloPrecioPK();
		articuloPrecioPK.setEmpId(getEmpId());
		articuloPrecioPK.setArtId(e.getArtId());
		articuloPrecioPK.setPrecioBaseId(e.getPrecioBaseId());
