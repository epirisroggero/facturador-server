...
public class ReportesDAOImpl extends ServiceBase implements ReportesDAO {
    ...
    public List<Object[]> stockPrecio(final String[] listas, String tipoFiltro, final String proveedores, final String familias,  final String desde, final String hasta, boolean filtrarCeros, boolean filtrarNegativos) {
    	//TODO
    	...
    	for (int i = 0; i < listas.length; i++) {
			sb.append(
					", artpr" + i + ".ArtPrecio" +
					", artpr" + i + ".mndIdPrecio" + 
					", pv" + i + ".PrecioVentaUtilidad" +
					", pv" + i + ".PrecioVentaPorcentaje");
		}

		if (Usuario.USUARIO_SUPERVISOR.equals(permisoId) || Usuario.USUARIO_ADMINISTRADOR.equals(permisoId) || Usuario.USUARIO_VENDEDOR_DISTRIBUIDOR.equals(permisoId)) {
	    	sb.append(
	    			", artpr.ArtPrecio" +
					", artpr.mndIdPrecio");
		}
		...
		for (int i = 0; i < listas.length; i++) {
			sb.append(" LEFT JOIN Preciosventa pv").append(i).append(" ON pv").append(i).append(".precioVentaId = ").append(listas[i])
				.append(" and pv").append(i).append(".empId = a.empId");
			sb.append(" LEFT JOIN articulos4 artpr").append(i).append(" ON artpr").append(i).append(".precioBaseId = pv").append(i).append(".precioBaseId")
				.append(" and artpr").append(i).append(".empId = a.empId").append(" and artpr").append(i).append(".artId = a.artId");
				...
				for (int j = 0; j < listas.length; j++) {
					...
					if (precioBase != null) {
						precio = Preciosventa.calcularPrecio(precioBase,
								BooleanDozerConverter.toBooleanValue(resultSet.getString(base + 2)), resultSet.getBigDecimal(6),
								resultSet.getBigDecimal(base + 3));
					}
	...
	public List<uy.com.tmwc.facturator.entity.TipoCambio> getTipoCambios() {
		...
		for (TipoCambio tipoCambio : list) {
			result.add(new uy.com.tmwc.facturator.entity.TipoCambio(tipoCambio.getId().getDia(), new Mapper().map(tipoCambio.getMoneda(),
					Moneda.class), tipoCambio.getComercial()));
		}
		return result;
	}
	...
	public List<uy.com.tmwc.facturator.entity.TipoCambio> getUltimosTipoCambios() {
		@SuppressWarnings("unchecked")
		List<TipoCambio> result = em.createNamedQuery("TipoCambio.ultimosTC").setParameter("empId", getEmpId()).getResultList();
